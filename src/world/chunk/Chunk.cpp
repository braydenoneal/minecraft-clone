...
Chunk::Chunk(Position chunk_position) {
    ...
    for (int x = 0; x < CHUNK_SIZE; x++) {
        for (int z = 0; z < CHUNK_SIZE; z++) {
            ...
            float surface_y = vertical_scale + vertical_scale * noise_value - (float) (chunk_position.y * CHUNK_HEIGHT);

            for (int y = 0; y <= (int) std::round(surface_y) && y < CHUNK_HEIGHT; y++) {
                set(x, y, z, {1});
            }
        }
    }
...
Block Chunk::get(int block_x, int block_y, int block_z) const {
    return blocks[block_x * CHUNK_SIZE * CHUNK_HEIGHT + block_y * CHUNK_SIZE + block_z];
}

void Chunk::set(int block_x, int block_y, int block_z, Block block) {
    blocks[block_x * CHUNK_SIZE * CHUNK_HEIGHT + block_y * CHUNK_SIZE + block_z] = block;
}
